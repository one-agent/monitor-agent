...
| 维度        | 考察内容                                                       |
|-----------|------------------------------------------------------------|
| Agent 设计	 | 意图识别（区分业务咨询 vs 状态询问）、多任务调度（问答的同时进行监控）。                     |
| 技术能力	     | 知识库检索 (RAG)、Webhook 构造与发送、Apifox API 调用、Uptime Kuma 对接/模拟。 |
|  结果质量	    | 回答准确性、告警及时性、文档生成的规范性。                                      |
---
...
- 智能问答：必须基于提供的"嗨云知识库"回答，严禁产生幻觉。如果知识库中没有答案，需回答"知识库中未找到相关信息"。
